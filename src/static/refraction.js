var refraction = {
    ...
    selectionElementIdFieldElementIds: {
        'material-up': { density: 'rho-up', lameConstants: ['lambda-up', 'mu-up'], slowness: 'slowness-up', angle: 'angle-up' },
        'material-down': { density: 'rho-down', lameConstants: ['lambda-down', 'mu-down'], slowness: 'slowness-down', angle: 'angle-down' },
    },
    ...
    selectMaterial: function (event) {
        ...
        var physicalConstants = refraction.materialNamePhysicalConstants[materialName]
    ...
    changeAngle: function (event) {
        ...
        for (var selectionElementId in refraction.selectionElementIdFieldElementIds) {
            fieldElementIds = refraction.selectionElementIdFieldElementIds[selectionElementId]
    ...
    dragAngle: function (event) {
        ...
        var selectedAngle = Math.atan2(y, x) + offsetAngleFactor * Math.PI / 2
    ...
    fillOutputElements: function (selectionElementId) {
        var fieldIds = this.selectionElementIdFieldElementIds[selectionElementId]
    ...
    fillOtherAngle: function (selectionElementId) {
        ...
        for (var otherSelectionElementId in this.selectionElementIdFieldElementIds) {
            if (otherSelectionElementId !== selectionElementId) break
        }

        var otherFieldElements = this.getMaterialFieldElements(otherSelectionElementId)
        var otherSlowness = this.getWaveSlowness(otherFieldElements)

        var otherFieldIds = this.selectionElementIdFieldElementIds[otherSelectionElementId]
        var otherAngleId = otherFieldIds.angle
        var otherAngleElement = document.getElementById(otherAngleId)

        var otherAngle = Math.asin(slowness / otherSlowness * Math.sin(angle))
    ...
    getPolarCoordinates: function (padding) {
        ...
        var radiusUp = slownessUp / slownessMax * canvas.width / 2 - padding
        var radiusDown = slownessDown / slownessMax * canvas.width / 2 - padding

        var polarCoordinates = {
            up: [radiusUp, angleUp], down: [radiusDown, angleDown]
        }
    ...
    drawSemicircles: function (canvas, polarCoordinates) {
        ...
        context.beginPath()
        context.arc(canvas.width / 2, canvas.height / 2, radiusUp, 0, Math.PI, true)
        context.stroke()

        context.strokeStyle = this.colors.down
        context.beginPath()
        context.arc(canvas.width / 2, canvas.height / 2, radiusDown, 0, Math.PI, false)
        context.stroke()
    },
    ...
    drawWaves: function (canvas, polarCoordinates) {
        ...
        context.lineTo(canvas.width / 2 + radiusUp * Math.sin(angleUp), canvas.height / 2 - radiusUp * Math.cos(angleUp))
        context.stroke()

        context.strokeStyle = this.colors.down
        context.beginPath()
        context.moveTo(canvas.width / 2, canvas.height / 2)
        context.lineTo(canvas.width / 2 - radiusDown * Math.sin(angleDown), canvas.height / 2 + radiusDown * Math.cos(angleDown))
        context.stroke()
    },

    getMaterialFieldElements: function (selectionElementId) {
        var inputFieldIds = refraction.selectionElementIdFieldElementIds[selectionElementId]
